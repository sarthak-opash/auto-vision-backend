...
if not DAMAGE_MODEL_PATH.exists() and not DAMAGE_MODEL_PATH.with_suffix(".onnx").exists():
    st.error("❌ Damage model not found. Expected at `runs/damage/weights/best.pt` or `best.onnx`")
    st.stop()
...
part_model = load_damage_model(str(PART_MODEL_PATH)) if (PART_MODEL_PATH.exists() or PART_MODEL_PATH.with_suffix(".onnx").exists()) else None
